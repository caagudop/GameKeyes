...
public class GUIItem : MonoBehaviour
{
	...
	void Update () {
        //En cada tick tenemos que comprobar si la puerta está activa
        if (m_doorGameObject == null)
            return;
        if (!m_doorGameObject.activeInHierarchy) return;

        //Si es así, tenemos que pintar la imagen m_Image del mismo color que la puerta
        switch (m_DoorColor)
        {
        ...
        // Por último, para que el componente no haga chequeos inútiles 
