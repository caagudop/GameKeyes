...
public class Item : MonoBehaviour
{
    ...
    {
        audioSource.Play();
        if (morada && other.tag=="Player")
        {
            GameObject.FindGameObjectWithTag("disable").transform.GetChild(0).gameObject.SetActive(true);
            other.gameObject.GetComponent<Jugador>().finJuego = true;
    ...
    public void ActivateDoor(GameObject other)
    {
        ...
        if (other.tag == "Player")
        {
            GameObject.FindGameObjectWithTag("Player").GetComponent<Jugador>().updateBools(m_DoorColorToActivate.ToString());
            //Obtener componente GameManager del GameObject m_GameManager y guardarlo
            ...
            if (gmComp)
            {
                // Llamar directamente a la función ActivateDoor sobre el componente GameManager
                ...
                // Crear un nuevo GameObject (indicar que es necesario para crear un nuevo transform)
                ...
                // Llamar directamente a la función SetCurrentSpawnPoint sobre el componente GameManager
